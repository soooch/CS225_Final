...
int main(int argc, char * argv[]) {
  ...
  // test values
  //N = 5;
  //std::vector<double> M = {0, 0, 0, 0, 1, 0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 0, 1, 0.5, 0, 0, 0, 0, 0.5, 1, 0};
  ...
  double norm = std::reduce(R.cbegin(), R.cend());
  std::for_each(R.begin(), R.end(), [norm](double & val) { val /= norm; });
  
  // multiply M into R [iteration] times
  for (int iter = 0; iter < iterations; iter++) {
    std::vector<double> nR(R.size());
    // matrix vector multiplication
    for (size_t i = 0; i < N; i++) {
      nR[i] = std::transform_reduce(R.begin(), R.end(), M.begin() + i * N, 0.0);
    }
    R = nR;
  }
  /*
